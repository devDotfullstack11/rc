...
class User_model extends CI_Model {
           ...
           // echo $this->db->last_query(); exit;
            ...
            if(!empty($record_arr)){
                ...
                if ($record_arr['status'] == 0) {
                    $result = ['status' => '0'  ,'message' => 'Account is not active, please contact admin', 'reason' => 'Invalid Username or Password' ];
                   ...
                   $result = ['status' => '1' , 'key' => $record_arr['id'] ,'message' => 'Logged In Successfully!',
                        'profile' =>$record_arr
                    ];
                } else {
                    $result = ['status' => '0'  ,'message' => 'Invalid Username or Password', 'reason' => 'Invalid Username or Password' ];
                } 
        ...
        public function get_one($key){
            $details = $this->db->get_where('rc_users', array('id' => $key));
            $record_arr = $details->first_row('array');
            return $record_arr;
        }

        public function activate($data){
            $details = $this->db->get_where('rc_users', array('id' => $data['origin']));
            $record_arr = $details->first_row('array');
            $date = new DateTime($record_arr['created_at']);
            $date->modify("+36500 day");
            $validity =  $date->format("Y-m-d H:i:s");
            $this->db->update('rc_users',['validity' => $validity, 'has_subscribed' => 'Yes'], array('id' => $data['origin']));
            return '1';
        }

        public function change_password($data,$id){            
                 $details = $this->db->get_where('rc_users', array('id' => $id));
                 $record_arr = $details->first_row('array');
                 
            //if (password_verify($data['old_password'],$record_arr['password'])) {
            if (md5($data['old_password']) == $record_arr['password'] ) {
                $this->db->update("rc_users",['password' => md5($data['new_password'])], ['id' => $id]);
                return 1 ;         
            } else {
                return 2;
            } 
