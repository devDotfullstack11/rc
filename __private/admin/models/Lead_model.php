...
class Lead_model extends CI_Model {
        ...
        public function get_total($where = [])
        {
           ...
           $result = $this->db->count_all("leads");
        ...
        /*public function login($data){
            $details = $this->db->get_where('leads', array('email' => $data['email']));
          ...
          public function login($data){
            $details = $this->db->get_where('leads', array('email' => $data['email'],'is_deleted' => 0));
            $record_arr = $details->first_row('array');
            //pr($record_arr,1);
            if(count($details) > 0){
        ...
        public function get_one($key){
            $details = $this->db->get_where('leads', array('api_key' => $key));
            $record_arr = $details->first_row('array');
            return $record_arr;
        }

        public function activate($data){
            $details = $this->db->get_where('leads', array('id' => $data['origin']));
            $record_arr = $details->first_row('array');
            $date = new DateTime($record_arr['created_at']);
            $date->modify("+36500 day");
            $validity =  $date->format("Y-m-d H:i:s");
            $this->db->update('leads',['validity' => $validity, 'has_subscribed' => 'Yes'], array('id' => $data['origin']));
            return '1';
        }
